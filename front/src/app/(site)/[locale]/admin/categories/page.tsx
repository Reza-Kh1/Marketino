...
export default function AdminCategoriesPage() {
  ...
  const columns: ColumnDef<CategorysTypes>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="text-xs text-admin-text-muted font-mono">{row.original?._count?.products || 0}</span>
    },
    {
      accessorKey: 'isActive',
      id: 'isActive',
      header: 'وضعیت دسته',
      cell: ({ row }) => <span className="text-xs text-admin-text-muted font-mono">{row.original?.isActive ?
    ...
    },
    {
      ...
      id: 'sortOrder',
      header: 'اولویت بندی',
      cell: ({ row }) => <span className="text-xs text-admin-text-muted font-mono">{row.original.sortOrder}</span>
    },
    {
      accessorKey: 'icon',
      id: 'icon',
      header: 'آیکون',
      cell: ({ row }) => <span className="text-xs text-admin-text-muted font-mono">{row.original.icon}</span>
    },
    {
      id: 'actions',
      header: 'عملیات',
