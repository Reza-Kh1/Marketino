    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4" dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">{isRTL ? 'برای مشاهده پیام‌ها، لطفاً وارد شوید' : 'Please login to view messages'}</p>
          <Link href="/login" className="inline-flex items-center gap-2 px-8 py-3 rounded-2xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-all">
            {t.common.login} <ArrowLeft className="w-4 h-4" />
          </Link>
        </div>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto px-4 py-8" dir={isRTL ? 'rtl' : 'ltr'}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-black mb-2 flex items-center gap-3">
          <MessageSquare className="w-8 h-8" /> {t.chat.title}
        </h1>
        <p className="text-muted-foreground mb-8">
          {isRTL ? 'گفتگو با فروشندگان درباره محصولات' : 'Chat with sellers about products'}
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-card border border-border rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-border">
              <h2 className="font-bold text-sm">{t.chat.products}</h2>
            </div>
            ...
            {conversations.length > 0 ? (
                ...
                {conversations.map(conv => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-blue-500 flex items-center justify-center text-white text-lg shrink-0">
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">{t.chat.click_product}</p>
                <Link href="/products"
                  ...
                  <ShoppingBag className="w-4 h-4" /> {t.common.products}
                </Link>
              </div>
            )}
          </div>
            ...
            {activeConversation ? (
              <>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        {activeConversation.isOnline ? (
                          <><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {t.chat.online}</>
                        ) : (
                          <><span className="w-1.5 h-1.5 rounded-full bg-muted-foreground/40" /> {t.chat.offline}</>
                        )}
                      </p>
                    </div>
                  </div>
                    ...
                    <div key={msg.id} className={`flex ${msg.senderId === 'current_user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={cn(
                        ...
                        <p className={cn(
                          ...
                          {new Date(msg.createdAt).toLocaleTimeString(isRTL ? 'fa-IR' : 'en-US', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <form onSubmit={handleSend} className="p-4 border-t border-border flex gap-2">
                  ...
                  <button
                    ...
                    className="h-11 w-11 rounded-xl bg-gradient-to-r from-violet-500 to-blue-500 text-white flex items-center justify-center hover:shadow-lg transition-all disabled:opacity-50 shrink-0"
                  >
              ...
              <div className="flex-1 flex items-center justify-center p-8">
                <div className="text-center">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t.chat.title}</h3>
                  <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                    {conversations.length > 0
                      ? (isRTL ? 'یک گفتگو را از لیست انتخاب کنید' : 'Select a conversation from the list')
                      : (isRTL ? 'به صفحه محصولات بروید و روی دکمه گفتگو کلیک کنید' : 'Go to products and click the chat button')
                    }
                  </p>
                  {conversations.length === 0 && (
                    <Link href="/products"
