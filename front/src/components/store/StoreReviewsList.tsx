...
import { Star, CheckCircle2, MessageSquareReply, Package, Store, Clock, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useStoreReview } from '@/hooks/store.hook';
...
export default function StoreReviewsList({ className, getData, storeId }: StoreReviewsListProps) {
  ...
  const {
    ...
    isFetching,
    isFetchingNextPage
  } = useStoreReview(storeId, getData);
  useEffect(() => {
  ...
  return (
    <div className={cn('space-y-4', className)}>
      {reviewData?.pages.map((item) => {
        return item.storeReview.map((r, index) => {
