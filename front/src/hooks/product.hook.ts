...
const VARIANT_KEYS = {
    ...
    list: (productId: string) => [...VARIANT_KEYS.all, 'list', productId] as const,
} as const;

export function useVariant(productId: string) {
    return useQuery<VariantType[] | []>({
        queryKey: VARIANT_KEYS.list(productId),
        queryFn: () => ProductService.listVariant(productId),
...
export function useCreateVariant() {
    ...
    return useMutation({
        mutationFn: (data: FormVariantDTO) => ProductService.createVariant(data),
        onSuccess: () => {
            // چون prefix مشترک با list(productId) دارد، همه‌ی کوئری‌های
            // مربوط به variant (برای هر محصولی) باطل و رفرش می‌شوند.
            queryClient.invalidateQueries({ queryKey: VARIANT_KEYS.all });
            toast.success('محصول با موفقیت ایجاد شد');
        },
        onError: (error: any) => {
            const messages = error?.response?.data?.message;
            const list = Array.isArray(messages) ? messages : [messages ?? 'خطای نامشخص'];
            list.forEach((msg) => toast.error(msg, { position: 'top-center' }));
        },
    });
}
...
export function useUpdateVariant() {
    ...
    return useMutation({
        mutationFn: ({ id, data }: { id: string; data: FormVariantDTO }) =>
