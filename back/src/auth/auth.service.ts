...
export class AuthService {
   ...
   */
  async registerSellerOTP(dto: RegisterSellerOtpDto) {
    const contact = dto.email || dto.phone;
    const type = dto.email ? 'email' : 'phone';

    if (!contact) {
      throw new BadRequestException('ایمیل یا شماره تلفن الزامی است');
    }

    const cleanContact = type === 'phone' ? contact.replace(/^(\+98|98)/, '0') : contact;
    ...
    // Check email uniqueness if registering with email
    if (dto.email) {
      const existingEmail = await this.prisma.user.findUnique({
        where: { email: dto.email },
      });
    ...
    const user = await this.prisma.user.create({
      data: {
        username: dto.username,
        email: dto.email || `${cleanContact}@phone.user`,
        phone: type === 'phone' ? cleanContact : undefined,
        password: await bcrypt.hash(dto.password, 12),
        firstName: dto.firstName,
        lastName: dto.lastName,
        role: 'seller',
    ...
    // Create initial seller rating
    await this.prisma.sellerRating.create({
      data: {
        sellerId: user.id,
        avgRating: 0,
        totalReviews: 0,
